...
public class RsaService {
     ...
     */
    private static final long DEFAULT_RSA_EXPIRE_TTL = 24 * 60 * 60 * 1000L;
    ...
    private RSAUtil.RsaKeypair getRsaKeypair(String serviceCode) {
        if (StringUtils.isBlank(serviceCode)) {
            throw new GloamInternalException("获取密钥对失败！serviceCode不可为null");
        }
        // 先在缓存中获取
    ...
    private RSAUtil.RsaKeypair initRsaKeypair(String serviceCode) {
        if (StringUtils.isBlank(serviceCode)) {
            throw new GloamInternalException("初始化密钥对失败！serviceCode不可为null");
        }
        try {
            ...
            throw new GloamInternalException("初始化rsa keypair 失败:{}", e.getMessage());
        }
    }
