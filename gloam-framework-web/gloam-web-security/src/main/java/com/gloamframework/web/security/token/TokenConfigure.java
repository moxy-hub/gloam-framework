...
public class TokenConfigure {
    ...
    @Bean
    public RequestParseFilter requestParseFilter() {
        return new RequestParseFilter();
    }

    @Bean
    public TokenAuthenticationFilter tokenAuthenticationFilter(TokenManager tokenManager, SecurityProperties securityProperties) {
        return new TokenAuthenticationFilter(tokenManager, securityProperties.getToken());
    }
